...
**Use arrow functions for all business logic, utilities, and application code. Classes are only allowed when required by
frameworks.**
...
// Extract business logic from framework classes
...
### Framework Exception (Classes Allowed)

```typescript
// Colyseus requires extending Room class
export class GameRoom extends Room<GameState> {
  onCreate(options: GameOptions) {
    // Extract logic to arrow functions
    this.state = createInitialGameState(options);
  }

  onJoin(client: Client, options: any) {
    // Extract logic to arrow functions
    const player = createPlayer(client.sessionId, options);
    this.state.players.push(player);
  }
}

// Phaser requires extending Scene class
export class GameScene extends Phaser.Scene {
  create() {
    // Extract logic to arrow functions
    const maze = generateMaze(this.mazeConfig);
    this.renderMaze(maze);
  }
}

// Colyseus requires Schema classes for state synchronization
...
// NEVER use classes for business logic (only frameworks)
class Calculator {
...
- Framework classes are isolated and business logic is testable
...
- **NEVER include file extensions in import statements** (no `.js`, `.ts`, or `.tsx`)
...
| Classes            | Allowed only when required by frameworks              |
| Framework Classes  | Colyseus Room/Schema, Phaser Scene (extract logic)    |
...
| Import extensions  | Never include file extensions (`.js`, `.ts`, `.tsx`)  |
